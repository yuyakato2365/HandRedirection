...
namespace GaussianSplatting.Runtime
{
    ...
    public class GaussianSplatRenderer : MonoBehaviour
    {
        ...
        internal string GetDiagnosticState(Camera cam)
        {
            Vector3 localToCamera = cam != null ? cam.transform.InverseTransformPoint(transform.position) : Vector3.zero;
            return
                $"  - gaussian={name} activeSelf={gameObject.activeSelf} activeInHierarchy={gameObject.activeInHierarchy} " +
                $"enabled={enabled} isActiveAndEnabled={isActiveAndEnabled} registered={m_Registered} " +
                $"resourcesAreSetUp={resourcesAreSetUp} validAsset={HasValidAsset} validRenderSetup={HasValidRenderSetup} " +
                $"asset={(m_Asset != null ? m_Asset.name : "<null>")} assetSplats={(m_Asset != null ? m_Asset.splatCount : 0)} " +
                $"splatCount={m_SplatCount} renderMode={m_RenderMode} gpuPos={(m_GpuPosData != null)} " +
                $"gpuOther={(m_GpuOtherData != null)} gpuChunks={(m_GpuChunks != null)} pos={transform.position} " +
                $"localToCamera={localToCamera} scale={transform.lossyScale}";
        }
        ...
        public void OnEnable()
        {
            ...
            {
                Debug.LogWarning($"[GaussianSplatDiagnostics] OnEnable resourcesAreSetUp=false {GetDiagnosticState(Camera.main)}");
                return;
            }
