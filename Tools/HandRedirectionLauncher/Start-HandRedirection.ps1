...
$script:ringMapPixelsPerMeter=100.0;$script:ringMapCenterX=0.0;$script:ringMapCenterY=0.0;$script:dragRing=$null
...
function TargetRingLayoutStatus($s){$p=$s -split '\s+';if($p.Count-ne7-or$p[0]-ne'TARGET_RING_LAYOUT'){return};$id=$p[1];if(!$script:ringLayouts.ContainsKey($id)){return};$layout=$script:ringLayouts[$id];$layout.X=[double]$p[3];$layout.Z=[double]$p[4];$layout.RadiusX=[math]::Max(0.01,[double]$p[5]);$layout.RadiusZ=[math]::Max(0.01,[double]$p[6]);if($ringMap){$ringMap.Invalidate()};$ringPatternState.Text="Layout $id loaded: X=$($p[3]) Z=$($p[4]) m";$ringPatternState.ForeColor=$green}
...
$b=Button 'Load Settings + Layout' 24 758 190 40;$b.Add_Click({Send 'GET_TARGET_RING_SETTINGS A';Send 'GET_TARGET_RING_SETTINGS B';Send 'GET_TARGET_RING_SETTINGS C';Send 'GET_TARGET_RING_LAYOUT'});$rc.Controls.Add($b)
